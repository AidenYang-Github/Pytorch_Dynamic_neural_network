...
if __name__ == "__main__":
    x = torch.unsqueeze(torch.linspace(-1.4, 1.4, 200), dim=1)  # x data (tensor), shape=(100, 1)
    # y_1 = x.pow(2) + 0.2 * torch.rand(x.size())  # noisy y data (tensor), shape=(100, 1)
    y_1 = np.sin(x) + 0.2 * torch.rand(x.size())  # noisy y data (tensor), shape=(100, 1)
    ...
    y_3 = np.tan(x) + 0.2 * torch.rand(x.size())

    net_1 = Net(n_feature=1, n_hidden=500, n_output=1)
    optimizer_1 = torch.optim.SGD(net_1.parameters(), lr=0.05, momentum=0.7)  # 传入 net 的所有参数, 学习率
    ...
    optimizer_3 = torch.optim.SGD(net_3.parameters(), lr=0.05, momentum=0.7)  # 传入 net 的所有参数, 学习率
