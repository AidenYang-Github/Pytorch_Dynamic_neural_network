...
# y_2 = x.pow(2) + 0.2 * torch.rand(x.size())  # noisy y data (tensor), shape=(100, 1)
...
# == 训练网络 ==
# optimizer 是训练的工具
optimizer_1 = torch.optim.SGD(net_1.parameters(), lr=0.01)  # 传入 net 的所有参数, 学习率
...
optimizer_2 = torch.optim.SGD(net_2.parameters(), lr=0.01)  # 传入 net 的所有参数, 学习率
loss_func_2 = torch.nn.MSELoss()  # 预测值和真实值的误差计算公式 (均方差)
plt.figure()
for t in range(2000):
    prediction_2 = net_2(x)  # 喂给 net 训练数据 x, 输出预测值
    loss_2 = loss_func_2(prediction_2, y_1)  # 计算两者的误差
    ...
    if t % 10 == 0:
        # plot and show learning process
        print(t, float('%0.6f' % loss_2.data.numpy()))
        plt.cla()
        plt.grid()
        plt.scatter(x.data.numpy(), y_1.data.numpy())
        plt.plot(x.data.numpy(), prediction_2.data.numpy(), 'b-', lw=2)
        plt.text(0.5, 0.1, 'Loss=%.6f' % loss_2.data.numpy(), fontdict={'size': 20, 'color': 'blue'})
        plt.pause(0.1)
# """
