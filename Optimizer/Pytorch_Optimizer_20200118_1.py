...
if __name__ == '__main__':
    ...
    for t in range(20001):
        ...
        if t % 100 == 0:
            ...
            plt.text(-0.25, 1, 'Loss_SGD=%.6f' % loss_SGD.data.numpy(), fontdict={'size': 20, 'color': 'red'})
            plt.text(-0.25, 0.8, 'loss_Momentum=%.6f' % loss_Momentum.data.numpy(),
                     fontdict={'size': 20, 'color': 'yellow'})
            plt.text(-0.25, 0.6, 'loss_RMSprop=%.6f' % loss_RMSprop.data.numpy(),
                     fontdict={'size': 20, 'color': 'green'})
            plt.text(-0.25, 0.4, 'loss_Adam=%.6f' % loss_Adam.data.numpy(), fontdict={'size': 20, 'color': 'magenta'})
